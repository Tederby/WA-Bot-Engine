...
/**
 * Invalidate group metadata cache for a specific group or all groups.
 * @param {string} [chatId]
 */
export function invalidateGroupMetadataCache(chatId) {
    if (chatId) {
...
export async function buildContext(message, sock) {
    ...
    if (isGroup) {
        ...
        // Sanitize sender — strip device ID and resolve to canonical PN
        ...
        // Also sanitize sender in private chats (strip device ID) & resolve to canonical PN
        if (sender && sender.includes(":")) {
            sender = sender.split(":")[0] + (sender.includes("@") ? "@" + sender.split("@")[1] : "");
        }
        sender = resolveUserId(sender);
