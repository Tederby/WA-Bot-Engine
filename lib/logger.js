 ...
 *
 * Instead of inline console.log with manual color/formatting,
 ...
 * Uses native Intl.DateTimeFormat — no external dependency needed.
...
export const logger = {
    ...
    /**
     * Log an authorization attempt (e.g. owner commands).
     */
    auth(timestamp, label, pushname, isGroup, groupName, status) {
        const statusColor = status === "ALLOWED" ? "green" : "red";
        console.log(
            color("[AUTH]", "magenta"),
            color(fmtTime(timestamp), "yellow"),
