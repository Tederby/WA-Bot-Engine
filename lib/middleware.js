...
export function checkPermissions(cmd, ctx) {
    // [OPTIONAL] Group-level user ban — remove if you don't use the built-in ban system
    ...
    // [OPTIONAL] Bot admin role — remove if you don't use the built-in admin system
    ...
    if (cmd.ownerOnly && !ctx.isOwner) {
        if (cmd.silentDrop) return "SILENT_DROP";
